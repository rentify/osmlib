...
OSMLib provides a way to integrate Ruby and Rails applications to OpenStreetMap.

This gem has support to:

- Handling the basic elements of OpenStreetMap: Node, Way, Relation, Changeset;
- Access the API;
- Database object to locally store elements;
- Parse OpenStreetMap XML files (.osm);
- Parse OpenStreetMap Change files (.osc).
...
Users mailing list: osmlib-user@rubyforge.org[http://rubyforge.org/mailman/listinfo/osmlib-user]

Documentation: http://vgeorge.github.com/rgeo/rdoc

Report issues: http://github.org/vgeorge/osmlib/issues

OpenStreetMap Wiki (outdated): http://wiki.openstreetmap.org/wiki/Osmlib


=== Development

Development mailing list: osmlib-devel@rubyforge.org[http://rubyforge.org/mailman/listinfo/osmlib-devel]
